# -*- coding: utf-8 -*-
"""
Полная выгрузка данных ГТЛ из текущего проекта QGIS в Excel — версия 5.
...
- листы периодов формируются по логическим маршрутам и датам;
...
def main():
    ...
    default_path = str(Path(desktop) / "Выгрузка_ГТЛ_из_QGIS_v5.xlsx")
    ...
    add_error(
        ...
        "",
        "Зоны выхода",
        "Зона выхода берётся из OutZones.Number.",
    )
    add_error(
        "Правила выгрузки",
        "",
        "ПХГ",
        "Для ПХГ ищется каждый сегмент GasPipeSegments, в котором UUID "
        "ПХГ указан как StartGtsPointId или EndGtsPointId. Газопровод берётся "
        "из GasPipelines.Name, километр — из StartKilometer, расстояние — "
        "из Length, точка подключения — противоположный GtsPoints.Name.",
    )
    ...
    ws_phg_control = create_sheet(
        ...
        [
            ...
            "StartKilometer",
            "Length",
            "UUID точки подключения",
            "Точка подключения",
            "Количество найденных связей у ПХГ",
        ],
    )
    ...
    for feature, data in specialized_rows("UndergroundGasStorages"):
        ...
        # ПХГ без прямой связи не удаляется.
        if not connections:
            connections = [{
                "segment_id": None,
                "pipeline_name": None,
                "start_kilometer": None,
                "length": None,
            ...
            add_error(
                "ПХГ",
                storage_id,
                "Нет прямого сегмента",
                "UUID ПХГ не найден ни в StartGtsPointId, "
                "ни в EndGtsPointId таблицы GasPipeSegments.",
            )

        # Если найдено несколько сегментов, сохраняем каждый вариант
        # отдельной строкой. Совпадающие названия ПХГ допустимы.
        if len(connections) > 1:
            add_error(
                "ПХГ",
                storage_id,
                "Несколько прямых связей",
                f"Найдено примыкающих сегментов: {len(connections)}. "
                "Каждая связь выгружена отдельной строкой.",
            )
        ...
        for connection in connections:
            rows.append(
                [
                    ...
                    data["name"],
                    region_name(
                        attr(storage_layer, feature, "RegionId")
                    ),
                    data["transgas"],
                    data["latitude"],
                    data["longitude"],
                    ...
                    connection["start_kilometer"],
                    connection["length"],
                    connection["connected_point_name"],
    ...
    rules_rows = [
        ...
        [
            "Зоны",
            "InZones / OutZones",
            "InZoneId / OutZoneId -> Id; для выхода отображается OutZones.Number.",
            "Номер зоны выхода.",
        ],
        [
            "ПХГ",
            "GasPipeSegments",
            "UUID ПХГ совпадает с StartGtsPointId или EndGtsPointId.",
            "GasPipelines.Name -> магистральный газопровод; "
            "StartKilometer -> км подключения; Length -> расстояние; "
            "противоположный GtsPoints.Name -> точка подключения.",
        ],
        [
            "ГИС/БП",
            "ConnectionKilometer",
